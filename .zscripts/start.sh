...
if [ -f "./next-service-dist/server.js" ]; then
    ...
    if [ "$DATABASE_URL" = "$DEFAULT_PACKAGED_DATABASE_URL" ]; then
        if [ ! -f "$DEFAULT_PACKAGED_DB_PATH" ]; then
            echo "⚠️  No se encontró $DEFAULT_PACKAGED_DB_PATH. Intentando crear base de datos..."
            mkdir -p /app/db
            DATABASE_URL="file:$DEFAULT_PACKAGED_DB_PATH" npx prisma db push --skip-generate 2>/dev/null || true
            if [ -f "$DEFAULT_PACKAGED_DB_PATH" ]; then
                echo "✅ Base de datos creada. NOTA: sin usuarios seed. Ejecuta 'npm run seed' para crear usuarios."
            else
                echo "❌ No se pudo crear la base de datos. La aplicación arrancará pero el login no funcionará."
            fi
        fi

        echo "🗄️  Base de datos: $DEFAULT_PACKAGED_DB_PATH"
    else
