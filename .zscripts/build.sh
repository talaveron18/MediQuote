...
# Ensure database exists and is seeded before building
if [ ! -f "./db/custom.db" ]; then
    echo "🗄️  No se encontró db/custom.db. Ejecutando setup (generate + push + seed)..."
    bun run setup
    echo "✅ Setup completado."
fi

if [ -f "./db/custom.db" ]; then
    echo "🗄️  复制 base de datos al paquete de construcción..."
    mkdir -p "$BUILD_DIR/db"
    cp ./db/custom.db "$BUILD_DIR/db/custom.db"

    echo "🗄️  Verificando estructura de base de datos en paquete..."
    DATABASE_URL="file:$BUILD_DIR/db/custom.db" bun run db:push
    echo "✅ Base de datos del paquete lista"
    ls -lah "$BUILD_DIR/db"
else
    echo "❌ Error: no se pudo crear db/custom.db. Verifica que prisma y seed funcionen."
    exit 1
fi
