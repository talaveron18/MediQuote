...
function privateDownload(body: BodyInit, contentType: string, filename: string) {
  ...
  try {
    ...
    if (!process.env.DATABASE_URL?.startsWith('file:')) {
      ...
      if (type === 'download' || type === 'export') {
        ...
        return privateDownload(JSON.stringify(backup), 'application/json', filename);
      }
    }
    if (type === 'list') return privateNoStoreJson({ backups: await listBackups() });
    if (type === 'download') {
      const filename = request.nextUrl.searchParams.get('filename') || '';
      if (!filename.trim()) return privateNoStoreJson({ error: 'Falta el archivo de copia' }, { status: 400 });
      const bytes = await readBackup(filename);
      return privateDownload(Uint8Array.from(bytes).buffer, 'application/vnd.sqlite3', filename);
    }
    return privateNoStoreJson({ error: 'Tipo no válido' }, { status: 400 });
  } catch {
    return privateNoStoreJson({ error: 'No se pudo completar la operación de backup' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!process.env.DATABASE_URL?.startsWith('file:')) {
      ...
      if (type === 'import') {
        ...
        try {
          ...
          return privateNoStoreJson({ error: 'La copia central no contiene JSON válido' }, { status: 400 });
        }
        if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
          return privateNoStoreJson({ error: 'La copia central no tiene un formato válido' }, { status: 400 });
        }
        const result = await importCentralDatabase(payload as Parameters<typeof importCentralDatabase>[0]);
    ...
    await logAudit({ action: 'backup_failed', entity: 'system', userId: auth.id, userName: auth.name, userRole: auth.role, result: 'error', errorMessage: 'Backup operation failed' }).catch(() => {});
    return privateNoStoreJson({ error: 'No se pudo completar la operación de backup' }, { status: 500 });
  }
}
