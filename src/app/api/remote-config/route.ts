...
export async function GET(request: Request) {
  ...
  if (type === 'audit') {
    ...
    if (auth instanceof NextResponse) return auth;
    try {
      const logs = await db.configAuditLog.findMany({ orderBy: { createdAt: 'desc' }, take: 50 });
  ...
  if (type === 'export') {
    ...
    try {
      ...
      await logConfigAudit({ action: 'export', userEmail: auth.email, role: auth.role, newVersion: config._meta.version, fileSource: REMOTE_CONFIG_PATH, result: 'success' });
      ...
      try {
        await logConfigAudit({
          action: 'export', userEmail: auth.email, role: auth.role, result: 'error',
          errorMessage: error instanceof Error ? error.message : 'Error no identificado', fileSource: REMOTE_CONFIG_PATH,
        });
...
export async function POST(request: Request) {
  ...
  if (type === 'import') {
    // El flujo aprobado permite al comercial importar la configuración previamente
    // generada por administración. La identidad procede siempre de la sesión.
    const auth = await requireAuth(request);
    if (auth instanceof NextResponse) return auth;
    ...
    try {
      ...
      if (readError || !data) {
        await logConfigAudit({
          action: 'import', userEmail: auth.email, userName: auth.name, role: auth.role,
          result: 'error', errorMessage: readError || 'No se pudo leer el archivo.', fileSource: REMOTE_CONFIG_PATH,
        });
      ...
      if (!validation.valid) {
        await logConfigAudit({
          action: 'import', userEmail: auth.email, userName: auth.name, role: auth.role,
          oldVersion, newVersion: validation.configVersion, result: 'error',
          errorMessage: validation.errors.join('; '), fileSource: REMOTE_CONFIG_PATH,
        });
        return privateNoStoreJson({ error: 'Validación fallida.', errors: validation.errors, warnings: validation.warnings }, { status: 422 });
      }

      const result = await applyRemoteConfig(data);
      await logConfigAudit({
        action: 'import', userEmail: auth.email, userName: auth.name, role: auth.role,
        oldVersion, newVersion: data._meta?.version, changesApplied: result.changesApplied as any,
        fileSource: REMOTE_CONFIG_PATH, result: result.success ? 'success' : 'error',
      ...
      try {
        await logConfigAudit({
          action: 'import', userEmail: auth.email, userName: auth.name, role: auth.role,
          result: 'error', errorMessage: error instanceof Error ? error.message : 'Error no identificado', fileSource: REMOTE_CONFIG_PATH,
        });
