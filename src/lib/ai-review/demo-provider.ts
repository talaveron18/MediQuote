...
function finding(id: string, title: string, detail: string, severity: 'info' | 'warning' | 'high' | 'critical', recommendation: string) {
  return { id, title, detail, severity, evidence: ['Foto económica del presupuesto', 'Datos del servicio declarados'], recommendation, requiresHumanValidation: severity !== 'info' };
}
  ...
  return [
    {
      ...
      findings: [finding('ges-vacaciones', 'Cobertura de vacaciones y ausencias', 'El precio no acredita una bolsa específica de sustitución ni el calendario de vacaciones.', 'warning', 'Contrastar con la gestoría el coste real de sustitución y documentar la causa contractual.')],
    },
    {
      ...
      findings: [finding('fin-absentismo', 'Sensibilidad al absentismo', `El coste base declarado es ${snapshot.baseCost.toFixed(2)} € y no incluye un escenario explícito de absentismo.`, 'high', 'Auditar el escenario del 3–8 % y fijar un umbral mínimo de margen antes de emitir.')],
    },
    {
      ...
      findings: [
        finding('aud-turno', 'Continuidad del turno y descansos', 'Un único profesional durante todo el periodo deja el servicio expuesto a descansos, ausencias e incidencias.', 'critical', 'Definir cobertura de relevo y probar el calendario completo con la plantilla mínima.'),
        finding('aud-festivos', 'Festivos territoriales y municipales', `El servicio se presta en ${snapshot.municipality}; debe aplicarse únicamente su calendario nacional, autonómico, provincial y municipal.`, 'high', 'Confirmar los dos festivos locales y el convenio aplicable a Toledo.'),
      ],
    },
    {
      ...
      findings: [
        finding('leg-control', 'Dirección y control operativo', 'La relación con el cliente debe evitar instrucciones directas que desdibujen quién organiza el trabajo.', 'high', 'Definir por contrato la interlocución, coordinación y facultades organizativas de GASI.'),
        finding('leg-datos', 'Protección de datos sanitarios', 'No consta si el profesional tratará datos de salud por cuenta del cliente.', 'warning', 'Determinar roles RGPD y formalizar encargo de tratamiento si corresponde.'),
      ],
    },
  ];
}
...
export function buildDemoBundle(snapshot: BudgetSnapshot): ReviewBundle {
  ...
  return { mode: 'demo', generatedAt: new Date().toISOString(), reviews, verdict: buildJointVerdict(reviews), prosecutor: buildProsecutorView(reviews) };
}
