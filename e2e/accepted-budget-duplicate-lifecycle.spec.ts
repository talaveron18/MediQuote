...
type Artifact = { artifactHash: string };
type SavedPayload = { budget: SavedBudget; immutableArtifact?: Artifact };
...
function material(name: string, unitPrice: number, quantity: number) {
  return {
    ...
    unitType: 'unidad', quantity, pricePerHour: unitPrice, fixedPrice: unitPrice, ivaPercent: 21,
  };
}
...
async function createAcceptedSource(page: Page, marker: string) {
  ...
  expect(saved.body.immutableArtifact?.artifactHash).toMatch(/^[a-f0-9]{64}$/);
  const signature = await api<SignatureRequest>(page, '/api/signatures', 'POST', {
    budgetId: saved.body.budget.id, recipientEmail: 'accepted.duplicate@example.invalid',
  });
  ...
  const accepted = await api(page, '/api/public/signature', 'POST', {
    token, signerName: 'Cliente Duplicate E2E', signerEmail: 'accepted.duplicate@example.invalid', signatureData: validPng, consent: true,
  });
  expect(accepted.status).toBe(200);
  return { budget: saved.body.budget, sourceArtifact: saved.body.immutableArtifact! };
}
...
async function calculateAndSaveDuplicate(page: Page, marker: string) {
  ...
  const savedPayload = await saveResponse.json() as SavedPayload;
  ...
  const created = await db.budget.findFirstOrThrow({
    where: { description: `${marker} (Copia)` }, orderBy: { createdAt: 'desc' }, include: { serviceBlocks: { orderBy: { sortOrder: 'asc' } } },
  });
  return { created, artifact: savedPayload.immutableArtifact! };
}
...
test('4 · el duplicado no hereda auditorías y sella un artefacto propio distinto del original', async ({ page }) => {
  await login(page);
  const marker = `E2E duplicate artifact ${Date.now()}`;
  const { budget, sourceArtifact } = await createAcceptedSource(page, marker);
  ...
  const sourceAudit = await db.costAudit.create({
    data: { budgetId: budget.id, createdById: admin.id, estimatedCost: 100, actualCost: 100, deviationAmount: 0, deviationPercent: 0, notes: 'auditoría sintética solo del original' },
  });
  await openDuplicate(page, budget);
  const { created, artifact } = await calculateAndSaveDuplicate(page, marker);
  ...
  expect(await db.costAudit.findUnique({ where: { id: sourceAudit.id } })).not.toBeNull();
  expect(artifact.artifactHash).not.toBe(sourceArtifact.artifactHash);
});
